...
class LoginHandler(BaseLoginForm):
    ...
    async def ask_enter_code(self, user_code: str) -> None:
        ...
        self._manager.print(
            f"Open this link to login: https://www.twitch.tv/activate")
        self._manager.print(
...
class TrayHandler(BaseTrayIcon):
    ...
    def notify(self,
               message: str,
               title: str | None = None,
               duration: float = 10) -> asyncio.Task[None] | None:
        pass
...
class InventoryHandler(BaseInventoryOverview):
    ...
    def update_progress(drop: TimedDrop, label) -> tuple[str, str]:
        ...
        if drop.is_claimed:
            ...
            progress_text = _(
                "gui", "inventory",
                "minutes_progress").format(minutes=drop.required_minutes)
...
class ProgressHandler(BaseCampaignProgress):
    ...
    def _update(self):
        ...
        self._window.addstr(0, 0,
                            _("gui", "progress", "game") + ' ', curses.A_BOLD)
        self._window.addstr(self._campaign_game)
        self._window.addstr(1, 0,
                            _("gui", "progress", "campaign") + ' ',
                            curses.A_BOLD)
        self._window.addstr(self._campaign_name)
        self._window.addstr(2, 0,
                            _("gui", "progress", "campaign_progress") + ' ',
                            curses.A_BOLD)
        self._window.addstr(f"{self._campaign_percentage}")
        self._window.addstr(
            2, 30,
            _("gui", "progress",
              "remaining").format(time=self._campaign_remaining))
        self._window.addstr(
            3, 0, self._progress_bar(self._campaign_progress, WINDOW_WIDTH))

        self._window.addstr(5, 0,
                            _("gui", "progress", "drop") + ' ', curses.A_BOLD)
        self._window.addstr(self._drop_rewards)
        self._window.addstr(6, 0,
                            _("gui", "progress", "drop_progress") + ' ',
                            curses.A_BOLD)
        self._window.addstr(f"{self._drop_percentage}")
        self._window.addstr(
            6, 30,
            _("gui", "progress",
              "remaining").format(time=self._drop_remaining))
        self._window.addstr(
            7, 0, self._progress_bar(self._drop_progress, WINDOW_WIDTH))
    ...
    def print(self, message: str):
        ...
        for line in lines:
            for i in range(0, len(line),
                           max_length):  # Split te line by length
